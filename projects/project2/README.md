...
To test locally, run `dune runtest -f`. Besides the provided public tests, you will also find the file `student.ml` in `test/student/`, where you'll be able to add `Alcotest`-based tests of your own. You are free to use the public tests as inspiration for your own tests. If you would like to know how to run a single test, or if you have any questions about how to interpret test output, then you can refer to the [project 1 `README`](https://github.com/cmsc330fall25/fall25/blob/main/projects/project1/project1.md)

NOTE: Student tests do not run by default. To have these tests run with your other tests, uncomment out the dune file.
