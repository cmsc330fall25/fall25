...
1. Checkout the project [HERE](https://classroom.github.com/a/k6dVQCec) to create your repository - you will have to do this for every project. After you checkout the project, click the generated link displayed to bring you to the repository page.
...
For Windows users: If you get the error `bash: ./public.sh: /bin/bash^M: bad interpreter: No such file or directory`, do the following:
1. Confirm that you are in WSL and not the regular terminal, then confirm the filepath you are in starts with `~` and not `/mnt`. If this is not true, delete this project folder and re-clone in the WSL filesystem.
2. If you were in WSL, you will need to convert the file's file endings from CRLF to LF. One way to do this is by running `vim public.sh -c "set ff=unix" -c ":wq"`.
3. Then, run `git config --global core.autocrlf input` so that you don't have this issue in the future.
4. Try running `./public.sh` again.
